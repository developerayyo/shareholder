...
$(document).ready(function() {
  function format(d) {
    return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;"><tr><td>Full name:</td><td>' +
           d.name + "</td></tr><tr><td>Extension number:</td><td>" + d.extn +
           "</td></tr><tr><td>Extra info:</td><td>And any further details here (images etc)...</td></tr></table>"
  }
  function filterGlobal() {
    $(".search-api")
        .DataTable()
        .search($("#global_filter").val(), $("#global_regex").prop("checked"),
  ...
  var t = $(".add-rows").DataTable(), counter = 1;
  $("#addRow").on("click",
                  function() {
                    t.row
                        .add([
  ...
  $(".show-child-rows tbody").on("click", "td.details-control", function() {
    var tr = $(this).closest("tr"), row = tableChildRows.row(tr);
    row.child.isShown()
        ? (row.child.hide(), tr.removeClass("shown"))
        : (row.child(format(row.data())).show(), tr.addClass("shown"))
  });
  var multipleRowsTable = $(".selection-multiple-rows").DataTable();
  $(".selection-multiple-rows tbody")
      .on("click", "tr", function() { $(this).toggleClass("selected") }),
      $("#row-count").on("click", function() {
        alert(multipleRowsTable.rows(".selected").data().length +
              " row(s) selected")
      });
  ...
  var tableDynamically = $(".hide-columns-dynamically")
                             .DataTable({scrollY : "200px", paging : !1});
  $("a.toggle-vis")
      .on("click",
          function(e) {
            e.preventDefault();
